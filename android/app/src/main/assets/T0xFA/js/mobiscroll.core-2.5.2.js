...
(function ($) {
    ...
    function Scroller(elem, settings) {
        ...
        function generateWheelItems(i) {
			...
			if(i==0&&loop)
			{
				html += '<div id="dw-v-1" class="dw-li dw-v" data-val="' + 26 + '" style="height:' + hi + 'px;line-height:' + hi + 'px;"><div class="dw-i">' + warr[i][22] + '</div></div>';
				html += '<div id="dw-v-2" class="dw-li dw-v" data-val="' + 27 + '" style="height:' + hi + 'px;line-height:' + hi + 'px;"><div class="dw-i">' + warr[i][23] + '</div></div>';
			}
			if(i==1&&loop)
			{
				html += '<div id="dw-v-1" class="dw-li dw-v" data-val="' + 62 + '" style="height:' + hi + 'px;line-height:' + hi + 'px;"><div class="dw-i">' + "00" + '</div></div>';
				html += '<div id="dw-v-2" class="dw-li dw-v" data-val="' + 63 + '" style="height:' + hi + 'px;line-height:' + hi + 'px;"><div class="dw-i">' + 30 + '</div></div>';
			}
            for (j in warr[i]) {
                if (l % 20 == 0) {
                ...
                if (i==1&&loop) {
                	if (j % 2 == 0) {
                		html += '<div class="dw-li dw-v" data-val="' + j + '" style="height:' + hi + 'px;line-height:' + hi + 'px;"><div class="dw-i">' + "00" + '</div></div>';
                	} else {
                		html += '<div class="dw-li dw-v" data-val="' + j + '" style="height:' + hi + 'px;line-height:' + hi + 'px;"><div class="dw-i">' + "30" + '</div></div>';
                	}
			...
			if(i==0&&loop)
			{
				html += '<div id="dw-v-3" class="dw-li dw-v" data-val="' + 24 + '" style="height:' + hi + 'px;line-height:' + hi + 'px;"><div class="dw-i">' + warr[i][0] + '</div></div>';
				html += '<div id="dw-v-4" class="dw-li dw-v" data-val="' + 25 + '" style="height:' + hi + 'px;line-height:' + hi + 'px;"><div class="dw-i">' + warr[i][1] + '</div></div>';
			}
			if(i==1&&loop)
			{
				html += '<div id="dw-v-3" class="dw-li dw-v" data-val="' + 60 + '" style="height:' + hi + 'px;line-height:' + hi + 'px;"><div class="dw-i">' + "00" + '</div></div>';
				html += '<div id="dw-v-4" class="dw-li dw-v" data-val="' + 61 + '" style="height:' + hi + 'px;line-height:' + hi + 'px;"><div class="dw-i">' + "30" + '</div></div>';
			}
            html += '</div>';
            return html;
        }
        ...
        function setGlobals(t) {
            min = $('.dw-li', t).index($('.dw-li', t).eq(0));
            max = $('.dw-li', t).index($('.dw-li', t).eq(-1));
            ...
            if (event('validate', [dw, index, time]) !== false) {
                ...
                $('.dw-ul', dw).each(function (i) {
                    ...
                    if (!(cell.hasClass('dw-sel')) || sc) {
                        ...
                        // Scroll to position
						//2014/11/27 刘兴振修正，根据当前orig的值，判断，是否当前是否需要滚动
						if(orig>25&&i==0&&loop)
						{
							// Set valid value
							cell = $('.dw-li[data-val="' + 0 + '"]', t),
							that.temp[i] = cell.attr('data-val');

							// Add selected class to cell
							$('.dw-sel', t).removeClass('dw-sel');
							cell.addClass('dw-sel');
							that.scroll(t, i, 2, sc ? time : 0.1, sc ? orig : undefined);
						}
						else if(orig<2&&i==0&&loop)
						{
							// Set valid value
							cell = $('.dw-li[data-val="' + 23 + '"]', t),
							that.temp[i] = cell.attr('data-val');

							// Add selected class to cell
							$('.dw-sel', t).removeClass('dw-sel');
							cell.addClass('dw-sel');
							that.scroll(t, i, 25, sc ? time : 0.1, sc ? orig : undefined);
						}
						else if(i==0&&loop)
						{
                            that.scroll(t, i, v, sc ? time : 0.1, sc ? orig : undefined);
						}
						else if(orig>61&&i==1&&loop)
						{
							// Set valid value
							cell = $('.dw-li[data-val="' + 0 + '"]', t),
							that.temp[i] = cell.attr('data-val');
						...
						else if(orig<2&&i==1&&loop)
						{
							// Set valid value
							cell = $('.dw-li[data-val="' + 59 + '"]', t),
							that.temp[i] = cell.attr('data-val');

							// Add selected class to cell
							$('.dw-sel', t).removeClass('dw-sel');
							cell.addClass('dw-sel');
							that.scroll(t, i, 61, sc ? time : 0.1, sc ? orig : undefined);	
						}
						else
						{
						   if(loop==false)
        ...
        function position(check) {
            ...
            css.top = t < 0 ? 0 : t + (s.topValue != undefined?parseInt(s.topValue):0);
            css.left = l;
            d.css(css);
        ...
        */
        that.show = function (prevAnim) {
            ...
            event('onShow', [dw, v]);
			window.setTimeout(function(){isShowing = false;},100);
        };
        ...
        * Cancel and hide the scroller instance.
        */
        that.cancelBlank  = function () {
            if (that.hide(false, 'cancelBlank') !== false) {
                event('onCancelBlank', [that.val]);
            }
        };
        // 点击空白出关闭对话框 追加 End
